...
const getMessageReactions = async (
  ...
  reactions.forEach((reaction) => {
    ...
    reactionMap.set(reaction.emoji, {
      ...
      reacted:
        current?.reacted || reaction.userId === currentUserId || false,
    });
  });
 ...
 */
router.get("/channel/:channelId", authMiddleware, async (req: any, res) => {
  try {
    ...
    const formattedMessages = messages.map((message) => {
      ...
      message.reactions.forEach((reaction) => {
        ...
        reactionMap.set(reaction.emoji, {
          count: current ? current.count + 1 : 1,
          reacted:
            current?.reacted || reaction.userId === req.user.userId || false,
        });
      });
 ...
 * POST /messages/dm
 */
router.post("/dm", authMiddleware, async (req: any, res) => {
  try {
...
router.post("/:messageId/reactions", authMiddleware, async (req: any, res) => {
  try {
...
router.patch("/:messageId", authMiddleware, async (req: any, res) => {
  try {
    const { messageId } = req.params;
    ...
    if (updatedMessage.conversationId) {
      io.to(updatedMessage.conversationId).emit(
        "message:updated",
        updatedMessage,
      );
    }
...
router.delete("/:messageId", authMiddleware, async (req: any, res) => {
  try {
    const { messageId } = req.params;
